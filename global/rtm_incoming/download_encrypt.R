...
get_resource <- function(filename, folder, key) {
  message(sprintf("Downloading %s...", filename), appendLF = FALSE)
  dat <- folder$download(filename, raw())
  ret <- cyphr::encrypt_data(dat, key)
  message(crayon::green("OK"))
  ret
}


hash_content <- function(data, key) {
  bytes <- cyphr::decrypt_data(data, key)
  as.character(openssl::sha256(bytes))
}


file_cache <- function(name, folder, key = cyphr::data_key()) {
  force(key)
  ...
  fstorr::fstorr(
    path,
    purrr::partial(get_resource, folder = folder, key = key),
    purrr::partial(hash_content, key = key))
}
